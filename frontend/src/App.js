...
class App extends Component {
  ...
  refreshList = () => {
    axios
      .get("http://3.93.173.69:8000/api/todos/")
      .then((res) => {
  ...
  handleSubmit = (item) => {
    ...
    if (item.id) {
      axios
        .put(`http://3.93.173.69:8000/api/todos/${item.id}/`, item)
        .then((res) => this.refreshList());
      return;
    }
    axios
      .post("http://3.93.173.69:8000/api/todos/", item)
      .then((res) => this.refreshList());
  };

  handleDelete = (item) => {
    axios
      .delete(`http://3.93.173.69:8000/api/todos/${item.id}/`)
      .then((res) => this.refreshList());
  };
